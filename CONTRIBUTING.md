...
- **A `// task N` comment marks where each task's tests begin**, naming the task in `instructions.md` they belong to. The marker applies to every test that follows it, until the next marker, so one is needed only where the task changes — but the first must come before the first test. The test runner reads these and links each result to its task.
